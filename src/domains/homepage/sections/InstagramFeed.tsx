...
export default function InstagramFeed() {
  ...
  // Memoize slide generation to prevent recalculation on every render
  const slides = useMemo(() => {
    if (media.length === 0) return []

    // Duplicate slides to ensure seamless loop
    // Max slidesPerView is 5, so we need at least 15 slides for smooth looping
    let duplicatedSlides = [...media]
    while (duplicatedSlides.length > 0 && duplicatedSlides.length < 15) {
      duplicatedSlides = [...duplicatedSlides, ...media]
    }

    return duplicatedSlides.map((item, index) => ({
      ...item,
...
const InstagramVideo = React.memo(function InstagramVideo({ item }: { item: InstagramMedia }) {
  const videoRef = useRef<HTMLVideoElement>(null)
  const containerRef = useRef<HTMLDivElement>(null)
  const [hasError, setHasError] = useState(false)
  const [isVisible, setIsVisible] = useState(false)

  // Intersection Observer to only play videos when visible
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const attemptPlay = async () => {
      try {
        ...
        // Video play failed, but don't set error state
        // as it's common for autoplay to be blocked
      }
    }
